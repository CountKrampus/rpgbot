...
7. Setup browser (choose one):
   Option A: Install Termux-based headless Chromium from TUR
...
• Browser not found: Install the TUR package with
  'pkg install tur-repo' followed by 'pkg install chromium',
  then select 'Termux Chromium (headless)' in System Settings
...
# Optional: Install Chromium from the Termux User Repository (TUR)
read -p "Install Termux-based Chromium? (y/n) " -n 1 -r
echo
if [[ $REPLY =~ ^[Yy]$ ]]; then
    echo "Installing the Termux User Repository..."
